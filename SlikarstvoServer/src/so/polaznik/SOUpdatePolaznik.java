 ...
 * @author Tina
 */
public class SOUpdatePolaznik extends AbstractSO{
    ...
    protected void validate(AbstractDomainObject ado) throws Exception {
        ...
        for (Polaznik postojeciPolaznik : polaznici) {
            if (!postojeciPolaznik.getPolaznikID().equals(polaznik.getPolaznikID())) {
                if (postojeciPolaznik.getKontaktTelefon().equals(polaznik.getKontaktTelefon())) {
                    throw new Exception("Vec postoji polaznik sa ovim brojem telefona!");
                }
            }
        }
    }
